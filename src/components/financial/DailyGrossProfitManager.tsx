...
export const DailyGrossProfitManager: React.FC<DailyGrossProfitManagerProps> = ({
  ...
  const dailyFinancials = useMemo(() => {
    ...
    // Daily Expenses
    const standardExpensesTotal = dailyExpenses
      .filter((e) => !e.isInventoryAssetLoss)
      .reduce((acc, e) => acc + e.amount, 0);
    const writeOffScrapLosses = dailyExpenses
      .filter((e) => e.isInventoryAssetLoss || e.category === 'damage_loss' || e.paidTo?.includes('Inventory Scrap Write-Off'))
      .reduce((acc, e) => acc + e.amount, 0);
    const operatingExpensesTotal = standardExpensesTotal + writeOffScrapLosses;
